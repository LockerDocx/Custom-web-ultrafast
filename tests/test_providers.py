...
# ── model-specific parameters reach the request (the MVP-1 repair) ───────────


def test_extras_ride_the_openai_body(monkeypatch):
    ...
    monkeypatch.setenv("POLICY_SEED", "11")
    monkeypatch.setenv("POLICY_STOP", '["END", "STOP"]')
    monkeypatch.setenv("POLICY_MAX_TOKENS", "2048")
    monkeypatch.setenv("POLICY_TEMPERATURE", "0.2")
    provider = providers.resolve("policy")
    _, _, body = providers.build_request(provider, "s", "u", 64)
    assert body["top_p"] == 0.8 and body["seed"] == 11 and body["stop"] == ["END", "STOP"]
    assert body["max_tokens"] == 2048  # the per-model budget replaces the caller's default
    assert body["temperature"] == 0.2


def test_extras_use_the_anthropic_names_and_nothing_else(monkeypatch):
    monkeypatch.setenv("POLICY_PROVIDER", "anthropic")
    monkeypatch.setenv("POLICY_MODEL", "claude-sonnet-4-5")
    monkeypatch.setenv("ANTHROPIC_API_KEY", "sk-ant")
    monkeypatch.setenv("POLICY_TOP_P", "0.5")
    monkeypatch.setenv("POLICY_STOP", '["END"]')
    provider = providers.resolve("policy")
    _, _, body = providers.build_request(provider, "s", "u", 64)
    assert body["top_p"] == 0.5 and body["stop_sequences"] == ["END"]
    assert "seed" not in body and "frequency_penalty" not in body


def test_stream_can_be_switched_off_per_model(monkeypatch):
    monkeypatch.setenv("POLICY_PROVIDER", "groq")
    monkeypatch.setenv("POLICY_MODEL", "openai/gpt-oss-20b")
    monkeypatch.setenv("GROQ_API_KEY", "gsk-test")
    monkeypatch.setenv("POLICY_STREAM", "false")
    seen = []
    monkeypatch.setattr(model, "post_json", lambda url, key, body, headers=None: seen.append(body) or {
        "choices": [{"message": {"content": "{}"}}],
    })
    monkeypatch.setattr(model, "post_stream", lambda *args, **kwargs: pytest.fail("streaming was disabled"))
    providers.chat(providers.resolve("policy"), "s", "u", max_tokens=64, on_delta=lambda chunk: None)
    assert seen and "stream" not in seen[0]


def test_extras_are_validated_before_any_request(monkeypatch):
    monkeypatch.setenv("POLICY_PROVIDER", "groq")
    monkeypatch.setenv("POLICY_MODEL", "m")
    monkeypatch.setenv("GROQ_API_KEY", "gsk-test")
    monkeypatch.setenv("POLICY_SEED", "not-a-number")
    with pytest.raises(ValueError, match="POLICY_SEED"):
        providers.resolve("policy")


def test_a_rejected_parameter_is_dropped_and_the_call_retries(monkeypatch):
    monkeypatch.setenv("POLICY_PROVIDER", "groq")
    monkeypatch.setenv("POLICY_MODEL", "m")
    monkeypatch.setenv("GROQ_API_KEY", "gsk-test")
    monkeypatch.setenv("POLICY_SEED", "3")
    bodies = []
    ...
    monkeypatch.setattr(model, "post_json", fake_post)
    providers.chat(providers.resolve("policy"), "s", "u", max_tokens=64)
