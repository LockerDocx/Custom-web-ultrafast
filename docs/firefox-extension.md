...
1. **Start the host** (from the repo root) — pick one:
   ...
   It prints `AI Agent for Firefox bridge: ws://127.0.0.1:8767` and waits.
...
3. Open the sidebar with the toolbar button (or `View → Sidebar → AI Agent`). The dot turns green when the bridge connects.
