#!/bin/bash
# Starter for the Jev Ultrafast Firefox bridge host (Linux).
# Prepares a private Python environment and runs the host. First run also
# creates the .env settings file. Run it from a file manager or a terminal.
...
if [ ! -f ".env" ]; then
  cp .env.example .env
  echo
  echo "A settings file .env was just created: $PWD/.env"
